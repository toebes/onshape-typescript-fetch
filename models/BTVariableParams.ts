/* tslint:disable */
/* eslint-disable */
/**
 * Onshape REST API
 * ## Welcome to the Onshape REST API Explorer  To use this API explorer, sign in to your [Onshape](https://cad.onshape.com) account in another tab, then click the **Try it out** button below (it toggles to a **Cancel** button when selected).  See the **[API Explorer Guide](https://onshape-public.github.io/docs/api-intro/explorer/)** for help navigating this API Explorer, including **[authentication](https://onshape-public.github.io/docs/api-intro/explorer/#authentication)**.  **Tip:** To ensure the current session isn\'t used when trying other authentication techniques, make sure to [remove the Onshape cookie](https://support.google.com/chrome/answer/95647#zippy=%2Cdelete-cookies-from-a-site) as per the instructions for your browser. Alternatively, you can use a private or incognito window.  ## See Also  * [Onshape API Guide](https://onshape-public.github.io/docs/): Our full suite of developer guides, to be used as an accompaniment to this API Explorer. * [Onshape Developer Portal](https://cad.onshape.com/appstore/dev-portal): The Onshape portal for managing your API keys, OAuth2 credentials, your Onshape applications, and your Onshape App Store entries. * [Authentication Guide](https://onshape-public.github.io/docs/auth/): Our guide to using API keys, request signatures, and OAuth2 in  your Onshape applications.
 *
 * The version of the OpenAPI document: 1.196.54436-927372740f35
 * Contact: api-support@onshape.zendesk.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
import type { BTConfiguredValue } from './BTConfiguredValue';
import {
    BTConfiguredValueFromJSON,
    BTConfiguredValueFromJSONTyped,
    BTConfiguredValueToJSON,
} from './BTConfiguredValue';

/**
 * 
 * @export
 * @interface BTVariableParams
 */
export interface BTVariableParams {
    /**
     * 
     * @type {BTConfiguredValue}
     * @memberof BTVariableParams
     */
    configuredDescription?: BTConfiguredValue;
    /**
     * 
     * @type {BTConfiguredValue}
     * @memberof BTVariableParams
     */
    configuredExpression?: BTConfiguredValue;
    /**
     * Variable description, if not configured
     * @type {string}
     * @memberof BTVariableParams
     */
    description?: string;
    /**
     * Variable definition expression, if not configured
     * @type {string}
     * @memberof BTVariableParams
     */
    expression?: string;
    /**
     * Variable name
     * @type {string}
     * @memberof BTVariableParams
     */
    name: string;
    /**
     * VariableType name, from FeatureScript VariableType
     * @type {string}
     * @memberof BTVariableParams
     */
    type: string;
}

/**
 * Check if a given object implements the BTVariableParams interface.
 */
export function instanceOfBTVariableParams(value: object): boolean {
    let isInstance = true;
    isInstance = isInstance && "name" in value;
    isInstance = isInstance && "type" in value;

    return isInstance;
}

export function BTVariableParamsFromJSON(json: any): BTVariableParams {
    return BTVariableParamsFromJSONTyped(json, false);
}

export function BTVariableParamsFromJSONTyped(json: any, ignoreDiscriminator: boolean): BTVariableParams {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'configuredDescription': !exists(json, 'configuredDescription') ? undefined : BTConfiguredValueFromJSON(json['configuredDescription']),
        'configuredExpression': !exists(json, 'configuredExpression') ? undefined : BTConfiguredValueFromJSON(json['configuredExpression']),
        'description': !exists(json, 'description') ? undefined : json['description'],
        'expression': !exists(json, 'expression') ? undefined : json['expression'],
        'name': json['name'],
        'type': json['type'],
    };
}

export function BTVariableParamsToJSON(value?: BTVariableParams | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        
        'configuredDescription': BTConfiguredValueToJSON(value.configuredDescription),
        'configuredExpression': BTConfiguredValueToJSON(value.configuredExpression),
        'description': value.description,
        'expression': value.expression,
        'name': value.name,
        'type': value.type,
    };
}

