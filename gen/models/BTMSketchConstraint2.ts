/**
 * Onshape REST API
 * The Onshape REST API consumed by all client. # Authorization The simplest way to authorize and enable the **Try it out** functionality is to sign in to Onshape and use the current session. The **Authorize** button enables other authorization techniques. To ensure the current session isn\'t used when trying other authentication techniques, make sure to remove the Onshape cookie as per the instructions for your particular browser. Alternatively, a private or incognito window may be used. Here\'s [how to remove a specific cookie on Chrome](https://support.google.com/chrome/answer/95647#zippy=%2Cdelete-cookies-from-a-site). - **Current Session** authorization is enabled by default if the browser is already signed in to [Onshape](/). - **OAuth2** authorization uses an Onshape OAuth2 app created on the [Onshape Developer Portal](https://dev-portal.onshape.com/oauthApps). The redirect URL field should include `https://cad.onshape.com/glassworks/explorer/oauth2-redirect.html`. - **API Key** authorization using basic authentication is also available. The keys can be generated in the [Onshape Developer Portal](https://dev-portal.onshape.com/keys). In the authentication dialog, enter the access key in the `Username` field, and enter the secret key in the `Password` field. Basic authentication should only be used during the development process since sharing API Keys provides the same level of access as a username and password.
 *
 * OpenAPI spec version: 1.171.24804-920f3dc76f2b
 * Contact: api-support@onshape.zendesk.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { BTMParameter1 } from '../models/BTMParameter1';
import { GBTConstraintType } from '../models/GBTConstraintType';
import { HttpFile } from '../http/http';

export class BTMSketchConstraint2 {
    'btType'?: string;
    'constraintType'?: GBTConstraintType;
    'drivenDimension'?: boolean;
    'entityId'?: string;
    'entityIdAndReplaceInDependentFields'?: string;
    'hasOffsetData1'?: boolean;
    'hasOffsetData2'?: boolean;
    'hasPierceParameter'?: boolean;
    'helpParameters'?: Array<number>;
    'importMicroversion'?: string;
    'namespace'?: string;
    'nodeId'?: string;
    'offsetDistance1'?: number;
    'offsetDistance2'?: number;
    'offsetOrientation1'?: boolean;
    'offsetOrientation2'?: boolean;
    'parameters'?: Array<BTMParameter1>;
    'pierceParameter'?: number;

    static readonly discriminator: string | undefined = undefined;

    static readonly attributeTypeMap: Array<{name: string, baseName: string, type: string, format: string}> = [
        {
            "name": "btType",
            "baseName": "btType",
            "type": "string",
            "format": ""
        },
        {
            "name": "constraintType",
            "baseName": "constraintType",
            "type": "GBTConstraintType",
            "format": ""
        },
        {
            "name": "drivenDimension",
            "baseName": "drivenDimension",
            "type": "boolean",
            "format": ""
        },
        {
            "name": "entityId",
            "baseName": "entityId",
            "type": "string",
            "format": ""
        },
        {
            "name": "entityIdAndReplaceInDependentFields",
            "baseName": "entityIdAndReplaceInDependentFields",
            "type": "string",
            "format": ""
        },
        {
            "name": "hasOffsetData1",
            "baseName": "hasOffsetData1",
            "type": "boolean",
            "format": ""
        },
        {
            "name": "hasOffsetData2",
            "baseName": "hasOffsetData2",
            "type": "boolean",
            "format": ""
        },
        {
            "name": "hasPierceParameter",
            "baseName": "hasPierceParameter",
            "type": "boolean",
            "format": ""
        },
        {
            "name": "helpParameters",
            "baseName": "helpParameters",
            "type": "Array<number>",
            "format": "double"
        },
        {
            "name": "importMicroversion",
            "baseName": "importMicroversion",
            "type": "string",
            "format": ""
        },
        {
            "name": "namespace",
            "baseName": "namespace",
            "type": "string",
            "format": ""
        },
        {
            "name": "nodeId",
            "baseName": "nodeId",
            "type": "string",
            "format": ""
        },
        {
            "name": "offsetDistance1",
            "baseName": "offsetDistance1",
            "type": "number",
            "format": "double"
        },
        {
            "name": "offsetDistance2",
            "baseName": "offsetDistance2",
            "type": "number",
            "format": "double"
        },
        {
            "name": "offsetOrientation1",
            "baseName": "offsetOrientation1",
            "type": "boolean",
            "format": ""
        },
        {
            "name": "offsetOrientation2",
            "baseName": "offsetOrientation2",
            "type": "boolean",
            "format": ""
        },
        {
            "name": "parameters",
            "baseName": "parameters",
            "type": "Array<BTMParameter1>",
            "format": ""
        },
        {
            "name": "pierceParameter",
            "baseName": "pierceParameter",
            "type": "number",
            "format": "double"
        }    ];

    static getAttributeTypeMap() {
        return BTMSketchConstraint2.attributeTypeMap;
    }

    public constructor() {
    }
}



